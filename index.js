...
client.on('voiceStateUpdate', async(oldState, newState) => {
  ...
  if (!oldState.channel) {
    ...
    if (dateDiff >= 600000) {
      ...
      client.channels.cache.get('Your-Guild-General-Channel-ID').send(`Hey <@${newState.member.id}>, Thank you for being active! You have got some **Delicacy Coins** for being active!`)
    }
  }
})
...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[Delicacy]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.on('ready',async () => {
    console.log("[Status]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'Delicacy | '+bots.length+' bots' }, status: "online" });
});
