...
module.exports.run = async (client,message,args) => {
   ...
   .setDescription(`*Searching your account*\n\n *This may take a second be patient*`)
   .setColor("#7289da")
   esitthis = await message.channel.send(embedsearch)
   await sleep(3000)
   const embed = new Discord.MessageEmbed()
   .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
   .setDescription(`*Searched Your account*\n\n*You Own* \`\`${bots.length}\`\` *Discord Bot(s) in Delicacy Monitoring*`)
   .setColor("#7289da")
