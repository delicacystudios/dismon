...
var bot = message.mentions.users.first()
    ...
    if(!bot)
    {
      ...
      .setDescription(`*Thats not a current bot in disbots or in guild.*`)
       .setColor("#7289da")
   ...
   if(!checking)
   {
    ...
    let checks = db.fetch(`checks_${bot.id}`); 
     ...
     const embed2 = new Discord.MessageEmbed()
      .setDescription(`Uptime - ${checking}% Checks - ${ochecks || 0}/${checks || 0} \n ${check} Time - ${days}d ${hour}h ${minutes}m ${seconds}s`)
       .setColor("#7289da")
