...
console.log("[Delicacy]: Admin/Botlist/Certificate Decline router loaded.");
...
app.post("/admin/certificate/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        ...
        client.users.cache.get(botdata.ownerID).send(`Your bot named **${bota.tag}** certificate application has been declined for\nreason: **${rBody['reason']}**`)
    });
