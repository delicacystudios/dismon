...
console.log("[Delicacy]: Admin/Botlist/Confirm Bot router loaded.");
...
app.get("/admin/confirm/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<@${botdata.ownerID}>'s bot named **${bota.tag}** has been approved by <@${req.user.id}>.`)
