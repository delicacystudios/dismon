...
console.log("[Delicacy]: Admin/Botlist/Decline Bot router loaded.");
...
app.post("/admin/decline/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<@${botdata.ownerID}>'s bot named **${botdata.username}** has been declined for\nreason: **${rBody['reason']}** `)
        client.users.cache.get(botdata.ownerID).send(`Your bot named **${botdata.username}** has been declined for\nreason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
    })
