...
console.log("[Delicacy]: Admin/CodeShare/Add router loaded.");
...
app.post("/admin/addcode", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.codelog).send(new Discord.MessageEmbed()
        ...
        .setDescription(`The user named **[${req.user.username}](https://monitoring.delicacysound.repl.co/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
        .addField("Code Link", `https://monitoring.delicacysound.repl.co/code/${kod}`, true)
