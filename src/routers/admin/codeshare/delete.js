...
console.log("[Delicacy]: Admin/CodeShare/Delete router loaded.");
...
app.post("/admin/deletecode/:code", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
        .setTitle("Code deleted!").setColor("GREEN").setFooter(`Powered By DelicacySound`)
        .setDescription(`The user named **[${req.user.username}](https://monitoring.delicacysound.repl.co/user/${req.user.id})** deleted the code named **${kodDataFind.codeName}**.`)
