...
module.exports = async (client) => {
  ...
  app.engine("Delicacy", ejs.renderFile);
  app.set("view engine", "Delicacy");
  ...
  app.get("/robots.txt", function(req, res) {
    ...
    res.send(`Sitemap: https://monitoring.delicacysound.repl.co/sitemap.xml`);
  });
  app.get("/sitemap.xml", async function(req, res) {
    let link = "<url><loc>https://monitoring.delicacysound.repl.co/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
      link += "\n<url><loc>https://monitoring.delicacysound.repl.co/bot/" + bot.botID + "</loc></url>";
    })
  ...
  app.get("/callback", passport.authenticate("discord", {
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://monitoring.delicacysound.repl.co/user/${a.id}) The user named **site** tried to log in but could not log in because he was blocked from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    })
    req.session.destroy(() => {
      res.json({
        login: false,
        message: "You have been blocked from Delicacy.",
        logout: true
      })
      req.logout();
    });
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://monitoring.delicacysound.repl.co/user/${a.id}) User named **site** logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
...
app.get("/admin/premium/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.guilds.fetch(serverdata.id).then(bota => {
    client.channels.cache.get(config.server.channels.botlog).send(`<@${serverdata.ownerID}>'s server **${bota.name}** has been promoted to **Premium**.`)
    client.users.cache.get(serverdata.ownerID).send(`Your server named **${bota.name}** has been promoted to **Premium**.`)
  });
...
app.get("/admin/premium/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.guilds.fetch(serverdata.id).then(bota => {
    ...
    client.users.cache.get(serverdata.ownerID).send(`Your server named **${bota.name}**'s Premium has been removed.`)
  });
...
app.get("/admin/boost/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    ...
    client.users.cache.get(botdata.ownerID).send(`Your bot named **${bota.tag}**'s boost has been removed.`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.get("/admin/server/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s server named **${botdata.name}** has been deleted by ${req.user.username}.`)
...
app.get("/admin/promote/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot named **${bota.tag}**'s promotion has been removed.`)
    client.users.cache.get(botdata.ownerID).send(`Your bot named **${bota.tag}**'s promotion has been removed.`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.get("/admin/boost/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot  **${bota.tag}** has been **Boosted**.`)
...
app.get("/admin/team", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  renderTemplate(res, req, "/admin/team.ejs", {
    req,
    roles,
    config,
    db
...
http.listen(3000, () => { console.log("[Delicacy]: Website running on 3000 port.") });
...
sleep(1050)
console.clear();
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: General routers loading...");
sleep(500);
...
/* Uptime System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Uptime system routers loading...");
sleep(500);
...
/* Profile System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Profile system routers loading...");
sleep(500);
...
/* Code Share System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Code Share system routers loading...");
sleep(500);
...
/* Botlist System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Botlist system routers loading...");
sleep(500);
...
/* Server List System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Serverlist system routers loading...");
sleep(500);
...
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Admin Panel system routers loading...");
sleep(500);
...
/* Bot System */
console.log(" ")
console.log('\x1b[36m%s\x1b[0m', "[Delicacy]: Bot system loading...");
