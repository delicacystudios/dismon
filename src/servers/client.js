...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[delicacy/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[delicacy/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    let serversdata = require("../../src/database/models/servers/server.js");
    const servers = await serversdata.find();
    serverClient.user.setPresence({ activity: { type: 'PLAYING', name: 's. | '+ servers.length+ ' servers' }, status: "dnd" });
});
