...
module.exports.run = async (client, message, args) => {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(`\n[Click to View your Profile!](https://monitoring.delicacysound.repl.co/user/${users.id})\n\n**Analytics**\n You own **${bots.length || "0"} Bots** and **${servers.length} Servers**`)
    .setColor("#7289da")
    .addField("Bots [" + bots.length + "]", `${!bots ? "" : bots.map(a => "<@" + a.botID + ">  **[**[View " + a.username + "](https://monitoring.delicacysound.repl.co/bot/" + a.botID + ")**]**").join("\n")}\n\n **Servers [${servers.length}]** \n${!servers ? "" : servers.map(b => b.name + " **[**[View " + b.name + "](https://monitoring.delicacysound.repl.co/server/" + b.id + ")**]**").join("\n")}`, true)
